    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="bg-card border-b border-border p-4 md:p-6 sticky top-0 z-10">
        <div className="max-w-3xl mx-auto">
          ...
          <h1 className="text-2xl md:text-3xl font-bold text-foreground">Inventory Audit</h1>
          <p className="text-sm md:text-base text-muted-foreground mt-1">Compare physical inventory with recorded stock</p>
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto p-4 md:p-6">
        {step === 1 ? (
          ...
          <div className="bg-card border border-border rounded-lg p-4 md:p-8 shadow-lg">
            <div className="mb-6">
              <h2 className="text-lg md:text-xl font-bold text-foreground mb-2">Step 1: Enter Physical Inventory</h2>
              <p className="text-muted-foreground text-sm md:text-base">Count your physical stock and enter the numbers below</p>
            </div>

            <form onSubmit={handleStep1Submit} className="space-y-6">
              {/* Audit Note */}
              <div>
                <label className="block text-foreground font-semibold mb-2 text-sm">Audit Notes (optional)</label>
                <textarea
              ...
              <div className="bg-muted/20 border border-muted rounded-lg p-4">
                <p className="text-foreground font-semibold mb-3 text-sm">Sample: Count these items</p>
                <div className="space-y-3">
                  {[1, 2, 3].map((i) => (
                    <div key={i} className="flex gap-3 items-end flex-col sm:flex-row">
                      <div className="flex-1 w-full">
                        <label className="block text-muted-foreground text-xs md:text-sm mb-1">Item {i}</label>
                        <input
                          type="text"
                      ...
                      <div className="w-full sm:w-28">
                        <label className="block text-muted-foreground text-xs md:text-sm mb-1">Count</label>
                        <input
                          type="number"
                          placeholder="0"
                          className="w-full bg-input border border-border rounded-lg px-3 py-2 text-foreground text-sm"
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-primary text-primary-foreground py-3 px-4 rounded-lg hover:bg-primary/90 transition font-bold disabled:opacity-50 text-base"
              >
